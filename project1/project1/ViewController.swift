...
class ViewController: UIViewController, AVAudioPlayerDelegate{
    ...
    @IBOutlet var iconSlider: UISlider!
    ...
    func makeAndFireTimer(){
        self.timer = Timer.scheduledTimer(withTimeInterval: 0.01, repeats: true, block: { [unowned self] (timer:
            Timer) in
    ...
    func addPlayPauseButton(){
        ...
        button.setImage(UIImage(named: "button_pause"), for: UIControlState.selected) //selected 상태에선 이미지가 변화
    ...
    func addProgressSlider() {
        ...
        slider.addTarget(self, action: #selector(self.sliderValueChaned(_:)), for: UIControlEvents.valueChanged)
    ...
    @IBAction func sliderImageChanged(_ sender: UISlider){
        print("slider image changed")
    }
